...
public class GlobalCollisionMap {
    private static final Logger log = LoggerFactory.getLogger(RuneUtilsPlugin.class);
    private static RoaringBitmap bitmap = init();

    static byte[] load() {
        try {
            InputStream is = GlobalCollisionMap.class.getResourceAsStream("com.spinplugins.RuneUtils/map");

            if(is == null) {
                throw new IOException("Failed to load map");
            }
    ...
    public static RoaringBitmap init() {
        ...
        try {
            ...
            if(bytes == null) {
                throw new IOException("No bytes to load into bitmap");
            }
    ...
    public static List<WorldPoint> findPath(WorldPoint p) {
        ...
        while (!queue.isEmpty()) {
            ...
            if (currentData.equals(p)) {
                ...
                ret.remove(0);
                log.info("Path took {}ms", System.currentTimeMillis() - start);
                return ret;
            }
            //north
